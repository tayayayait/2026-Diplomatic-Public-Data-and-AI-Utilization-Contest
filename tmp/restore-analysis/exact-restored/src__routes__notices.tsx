...
export const Route = createFileRoute("/notices")({
  head: () => ({ meta: [{ title: "怨듭? 紐⑸줉 ??DiploLife" }] }),
...
function NoticesPage() {
  ...
  return (
    <AppShell eyebrow="Notices" title="泥대쪟援??덉쟾 怨듭?">
      <div className="mx-auto max-w-4xl">
        
        {/* Header & Filters */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 stagger-child" style={{ "--index": 1 } as any}>
          <div>
            <h2 className="text-[15px] font-semibold text-muted-foreground">
              {profile?.country || "???멸퀎"} 愿???멸탳遺 怨듭??ы빆
            </h2>
            <p className="text-[13px] text-muted-foreground mt-1">珥?{notices.items.length}嫄댁쓽 ?덈궡媛 ?덉뒿?덈떎.</p>
          </div>
        ...
        <div className="space-y-4">
          {filteredNotices.length > 0 ? (
            filteredNotices.map((notice, idx) => (
              ...
              >
                <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${
                  notice.category === 'SAFETY' ? 'bg-amber-500/10 text-amber-500' :
                  notice.category === 'VISA' ? 'bg-green-500/10 text-green-500' :
                  'bg-blue-500/10 text-blue-500'
                }`}>
                  {notice.category === 'SAFETY' ? <Bell className="h-6 w-6" /> :
                   notice.category === 'VISA' ? <Megaphone className="h-6 w-6" /> :
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-[12px] font-bold px-2 py-0.5 rounded-full ${
                      notice.category === 'SAFETY' ? 'bg-amber-500/20 text-amber-700 dark:text-amber-400' :
                      notice.category === 'VISA' ? 'bg-green-500/20 text-green-700 dark:text-green-400' :
                      'bg-blue-500/20 text-blue-700 dark:text-blue-400'
                    }`}>
                      {notice.category === 'SAFETY' ? '?덉쟾怨듭?' : notice.category === 'VISA' ? '鍮꾩옄/?됱젙' : '?앺솢?덈궡'}
                    </span>
                    <span className="text-[13px] text-muted-foreground">{notice.publishedAt?.slice(0, 10)}</span>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center rounded-3xl border border-dashed border-border bg-surface-alt/50 stagger-child" style={{ "--index": 2 } as any}>
              <Bell className="h-10 w-10 text-muted-foreground/50 mb-4" />
              <p className="text-[15px] font-medium text-muted-foreground">議곌굔??留욌뒗 怨듭??ы빆???놁뒿?덈떎.</p>
            </div>
          )}
        </div>
