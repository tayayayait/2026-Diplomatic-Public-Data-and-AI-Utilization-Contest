...
- Gemini-guided Google matching preserves the Gemini draft's route order, theme, and stay duration where feasible, normalizes restaurant stops to `meal` and cafe stops to `snack`, and replaces user-visible place identity, coordinates, Place ID, rating, review count, opening status, price fields, and previous-leg route time/distance with Google data.
